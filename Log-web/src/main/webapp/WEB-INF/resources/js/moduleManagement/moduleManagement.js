...
Ext.define('moduleManagement.ModuleManagementModel', {
...
var store = new Ext.data.Store({
			autoLoad : true,
			autoSync : true,// 需要同步
			model : 'moduleManagement.ModuleManagementModel',
			proxy : {
				type : 'rest',
				url : './.json',
				reader : {
...
var moduleManagementGrid = new Ext.grid.GridPanel({
			id : 'moduleManagementGrid',
			plugins : [rowEditing],
			store : store,
			region : 'center',
			columnLines : true,
...
moduleManagementGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
	moduleManagementGrid.down('#delete').setDisabled(selections.length === 0);
		});
...
Ext.application({
			...
			launch : function() {
				Ext.create('Ext.container.Viewport', {
							layout : 'border',
							items : [moduleManagementForm, moduleManagementGrid]
						});
			}
		});
